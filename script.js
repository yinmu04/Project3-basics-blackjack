var MODE_CHOOSE_NUM_PLAYERS = "choose num of players";
var MODE_QUERY_PLAYER_NAME = "player name";
var MODE_COLLECT_BET = "collect bet";
var MODE_DEAL_CARDS = "deal cards";
var MODE_SCAN_FOR_BLACKJACK = "scan for blackjack";
var MODE_HIT_OR_STAND = "hit or stand";
var MODE_COMPUTER_PLAYS = "computer plays";
var MODE_DISPLAY_RESULTS = "display results";
...
var getHandSum = function (hand) {
  ...
  if (sum > sumLimit && acesInHand > 0) {
    for (let i = 0; i < acesInHand; i += 1) {
      sum -= 10;
...
var calBetWinnings = function (player) {
  ...
  if (isBlackjack(player.hand)) {
    ...
    winRate = player.bet * 1.5;

    // player.points += winRate;
    return winRate;
  }
  // else player did not win with blackjack, multiplier is 1 times of the bet rate
  // playerProfiles[currPlayerIndex].points += winRate;
  return winRate;
};

// Calculate a player losses a player incurs from losing a bet

var calBetLosses = function (player) {
...
var craftInstructionsForNextSteps = function () {
  ...
  if (currPlayerIsLastPlayer()) {
    return "All players have played. So, click submit to see computer's cards";
  }
  ...
  if (didComputerBust()) {
    ...
    for (var i = 0; i < playerProfiles.length; i += 1) {
      ...
      if (getHandSum(currPlayer.hand) > sumLimit) {
        ...
        var lostRate = calBetLosses(currPlayer);
        // Subtract the bet from the player's points
        currPlayer.points -= lostRate;
      ...
      var wonRate = calBetWinnings(currPlayer);
      //  Add the winnings to the player's points
      currPlayer.points += wonRate;
      // Update the string that will be printed
      gameResults += `${listIndex}. ${currPlayer.name}- Bet: ${currPlayer.bet} || Delta: + ${wonRate} points. || Total points: ${currPlayer.points}<br>`;
    }
